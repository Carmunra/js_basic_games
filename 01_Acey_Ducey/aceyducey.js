// By default:
// - Browsers have a windwon object
// - Node.js does not
...
function getRandomCard(){
    // In our game, the value of ACE is gretaer than face card;
    // instead of having the value of ACE be 1, we'll have it be 14.
    // So, we want to shift the range of random number from 1-13 to 2-14. 
    let min = 2;
    let max = 14;
    // Return reandom integer between two values, inclusive
...
function getCardValue(card){
    ...
    // If card value matches a key in faceOrface value;
...
print('ACEY-DUCEY IS PLAYES IN THE FOLLOWING MANNER');
print('THE DEALER (COMPUTER) DEALS TWO CARD FACE UP');
print('YOU HAVE AN OPTION TO BET OU NOT BET DEPENDING');
print('ON WHETHER ON NOT YOU FEEL THE CARD WILL HAVE');
...
async function main(){
    ...
    // Loop game forever
        while(true){
            ...
            print('HERE ARE YOUR NEXT TWO CARD: ');
            print(getCardValue(cardOne));
            print(getCardValue(cardTwo));
            print('');

            // Loop intil receiving a valid bet
            let validBet = false;
            while (!validBet){
                print('\nWHAT IS YOUR BET? ');
